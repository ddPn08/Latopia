...
from . import modules
from latopia.torch_utils import init_weights
...
class Vocoder(torch.nn.Module):
    def __init__(
        ...
        resblock_cls = modules.DilatedResBlock if resblock else modules.ResBlock
...
def load_nsf_hifigan(model_dir: str, gin_channels: Optional[int] = None):
    config_path = os.path.join(model_dir, "config.json")
    config = NsfHifiganConfig.parse_json(config_path)
    vocoder = Vocoder(
        initial_channel=config.upsample_initial_channel,
        resblock=config.resblock == "1",
